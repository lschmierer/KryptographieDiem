...
class Homomorphismus(UnveraenderbaresObjekt):
    ...
    def __init__(self, quelle, ziel, element=None, basishom=None):
        ...
        if (element == None and basishom == None) and \
            ...
            if not (type(self.quelle) == Ganzzahlring or type(self.quelle) == Restklassenring or
                ...
                raise (
                    "Homomorphismus ist für diesen Typ von Ringen nicht implementiert.")
            ...
            if type(self.quelle) == Ganzzahlring:
                ...
                if self.element != None or self.basishom != None:
                    raise RuntimeError(
                        "Der erste Ring ist Z, deshalb wird nur noch ein weiterer Ring benötigt.")
                ...
                if self.element != None or basishom != None:
                    raise RuntimeError(
                        "Der erste Ring ist ein Restklassenring, deshalb wird neben dem zweiten Ring nichts weiter benötigt.")

                if not ((type(self.ziel) == Restklassenring and
                         self.quelle.modulus % self.ziel.modulus == 0) or
                        (type(self.ziel) == Polynomring and self.ziel.basisring == self.quelle)):
                    raise RuntimeError(
                        "Konstruktion des Homomorphismus nicht möglich.")
                ...
                if element == None:
                    raise RuntimeError(
                        "Da die Quelle ein Polynomring ist, muss noch ein Ringelement angegeben werden.")
                if not ((self.ziel == Z and type(self.element) == int) or
                        (isinstance(self.element, RingElement) and self.element.ring == self.ziel)):
                    raise RuntimeError(
                        "Das dritte Objekt muss ein Element aus dem zweiten Ring sein.")
            ...
            # Genau diese Homomorphismen sind Beginn implementiert worden.
                ...
                if self.basishom == None:
                    if self.quelle.basisring == self.ziel or self.quelle == self.ziel or self.quelle.basisring == Z or isinstance(self.quelle.basisring, Restklassenring):
                        self.basishom = Homomorphismus(
                            self.quelle.basisring, self.ziel)

                    else:
                        raise RuntimeError(
                            "Es wird noch ein Ringhomomorphismus vom Grundring zum Ziel benötigt.")

                if not isinstance(self.basishom, Homomorphismus):
                    raise TypeError(
                        "Das vierte Objekt muss ein Ringhomomorphismus (Typ Homomorphismus oder KringelHomomorphismus) sein.")

                if not (self.basishom.quelle == self.quelle.basisring and self.basishom.ziel == self.ziel):
                    raise RuntimeError(
                        "Der Grundring-Homomorphismus passt nicht zum Basisring der Quelle oder nicht zum Ziel.")
                ...
                def homfunktion(e):
                    ...
                    for i in range(0, e.grad+1):
                        rueck += self.basishom.anwenden(
                            e.koeffizient(i))*exp_element
    ...
    def __str__(self):
        ...
        rueck = "Ringhomomorphismus von {0} zu {1}".format(
...
class KringelHomomorphismus(Homomorphismus):
    ...
    def __init__(self, hom1, hom2):
        ...
        if not hom1.quelle == hom2.ziel:
            raise RuntimeError(
                "Die Homomorphismen können nicht verknüpft werden.")
    ...
    def anwenden(self, e):  # pylint: disable=E0202
